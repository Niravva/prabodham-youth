...
<div class="modal-body">
    <form id="formEditTags" action="{{ route('tagsMaster.update', $tagsMaster->id) }}" method="POST"
        enctype="multipart/form-data">
        @method('PUT')
        <div class="form-group">
            <label>{{ __('Tag Name') }}*</label>
            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                value="{{ $tagsMaster->name }}" />
            @error('name')
...
<script>
    $(document).ready(function() {
        ...
        $("#formEditTags").submit(function(e) {
            e.preventDefault(); // avoid to execute the actual submit of the form.
            var formData = $('#formEditTags').serialize();
            ...
            $.ajax({
                ...
                url: "{{ route('tagsMaster.update', $tagsMaster->id) }}",
                data: formData,
                dataType: 'json',
